  ...
  try {
    ...
    const resp = await fetch('https://api.deepseek.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'deepseek-chat',
        messages: [
          { role: 'system', content: '你是一位资深的临床医学专家，精通人体解剖学和生理学。直接返回纯JSON，不要使用markdown代码块。' },
          { role: 'user', content: `【患者】${age}岁 ${sexCn}性（${group}）\n\n【异常指标】\n${desc}\n\n针对每个异常给出解剖学/生理学解读，必须体现该患者年龄和性别的人口统计学特征。\n\n直接返回纯JSON:\n{"items":[{"metric":"指标代码","metric_cn":"中文名","organ_system":"对应五行（木火土金水）","severity":"mild/moderate/severe","anatomical_context":"解剖学位置和功能","physiological_analysis":"病理生理学分析","demographic_specific":"针对${age}岁${sexCn}性的特殊说明","recommendation":"建议"}],"summary":"整体健康风险评估总结"}` }
        ],
        temperature: 0.3,
        max_tokens: 4000,
      }),
